...
(function () {
  ...
  function makeDreamSide(starters, formation, key, bench, subsLeft, captainId) {
    const name = window.I18N ? window.I18N.t('team.name') : window.CONFIG.TEAM_NAME;
    return { key, name, code: null, cup: null, dream: true, captainId: captainId || null,
  ...
  // the OVERALL hit from fatigue is capped (kept light) — the raw points still
  // drive the energy bar, but skill only ever drops by FATIGUE_PENALTY_MAX.
  function fatiguePenalty(state, id) {
    const cap = window.CONFIG.FATIGUE_PENALTY_MAX;
    const f = fatigueOf(state, id);
    return cap != null ? Math.min(cap, f) : f;
  }

  // effective overall after fatigue (clamped to a sane floor)
  function effOverall(player, state) {
    return Math.max(40, player.overall - fatiguePenalty(state, player.id));
  }

  // bonus to a player's effective overall from the captain: the captain himself,
  // and teammates drafted from his same historical selection (team + cup).
  function captainChemBoost(player, captain, config) {
    const C = config || window.CONFIG;
    if (!captain) return 0;
    if (player.id === captain.id) return C.CAPTAIN_OVR_BOOST || 0;
    if (player.team === captain.team && player.cup === captain.cup) return C.CHEMISTRY_OVR_BOOST || 0;
    return 0;
  }
  ...
  // a reserve who sat the match out rests FULLY (energy back to 100%).
  ...
  function updateFatigue(state, starters, bench, extraTime) {
    ...
    bench.forEach(p => {
      if (!startedIds.has(p.id)) next[p.id] = 0;   // a full rest clears all accumulated fatigue
    });
    return next;
  }
  ...
  // full in-match effective overall: base − fatigue(capped) − pressure (floored)
  function effInMatch(player, state, roundN, hasLeader) {
    return Math.max(40, player.overall - fatiguePenalty(state, player.id) - pressureOf(player, roundN, hasLeader));
  }
  ...
  window.TEAM = {
    ...
    initFatigue, fatigueOf, fatiguePenalty, effOverall, staminaPct, staminaLevel, updateFatigue, applyFatigue,
    pressureOf, effInMatch, captainChemBoost,
