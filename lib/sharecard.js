...
(function () {
  ...
  async function render(data) {
    ...
    ctx.fillText(champ ? T('share.cardChampion') : T('share.cardEliminated'), cx, 500);
    ...
    const modeTxt = data.mode === 'almanaque' ? T('share.modeAlmanac') : T('share.modeClassic');
    ctx.fillText(T('share.campaignLine') + ' · ' + modeTxt, cx, 610);
    ...
    ctx.fillText(T('share.achLine', { n: data.achCount, total: data.achTotal }), 160, y + 58);

    // footer
    ctx.textAlign = 'center'; ctx.fillStyle = FG3; ctx.font = '500 24px Archivo, sans-serif';
    ctx.fillText(T('share.footer'), cx, H - 50);
