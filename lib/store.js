...
(function () {
  ...
  // ---- PROFILE ----
  // `theme` is intentionally absent from the defaults: when unset, the app
  // derives it from `prefers-color-scheme` on the first visit (and persists
  // the choice on first toggle). `lang` defaults to CONFIG.DEFAULT_LANG.
  // Both additions are retro-compatible — old profiles stay valid.
  const DEFAULT_PROFILE = { achievements: [], sound: true, plays: 0, titles: 0,
    speed: (window.CONFIG && window.CONFIG.MATCH_SPEED_DEFAULT) || 'normal',
    lang: (window.CONFIG && window.CONFIG.DEFAULT_LANG) || 'pt' };
